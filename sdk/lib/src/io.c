...
void write_tohost_tid(uint32_t tid, uint32_t val)
{
	...
	{
	case 0:
		write_csr(CSR_TOHOST(0), val);
		break;
	case 1:
		write_csr(CSR_TOHOST(1), val);
		break;
	case 2:
		write_csr(CSR_TOHOST(2), val);
		break;
	case 3:
		write_csr(CSR_TOHOST(3), val);
		break;
	case 4:
		write_csr(CSR_TOHOST(4), val);
		break;
	case 5:
		write_csr(CSR_TOHOST(5), val);
		break;
	case 6:
		write_csr(CSR_TOHOST(6), val);
		break;
	case 7:
		write_csr(CSR_TOHOST(7), val);
		break;
	default:
		_fp_abort("Invalid thread id: %i\n", (int)tid);
	}
}
...
void fp_print_string(char *str)
{
	do
	{
		uart_send(UART0_BASE, (uint8_t)*str);
	} while (*str++ != '\0');
}
...
void gpo_write(uint32_t port, uint32_t val)
{
	...
	{
	case 0:
		write_csr(CSR_UARCH4, val);
		break;
	case 1:
		write_csr(CSR_UARCH5, val);
		break;
	case 2:
		write_csr(CSR_UARCH6, val);
		break;
	case 3:
		write_csr(CSR_UARCH7, val);
		break;
	default:
		_fp_abort("Invalid port: %i\n", (int)port);
	}
}
...
void gpo_set(uint32_t port, uint32_t mask)
{
	...
	{
	case 0:
		set_csr(CSR_UARCH4, mask);
		break;
	case 1:
		set_csr(CSR_UARCH5, mask);
		break;
	case 2:
		set_csr(CSR_UARCH6, mask);
		break;
	case 3:
		set_csr(CSR_UARCH7, mask);
		break;
	default:
		_fp_abort("Invalid port: %i\n", (int)port);
	}
...
void gpo_clear(uint32_t port, uint32_t mask)
{
	...
	{
	case 0:
		clear_csr(CSR_UARCH4, mask);
		break;
	case 1:
		clear_csr(CSR_UARCH5, mask);
		break;
	case 2:
		clear_csr(CSR_UARCH6, mask);
		break;
	case 3:
		clear_csr(CSR_UARCH7, mask);
		break;
	default:
		_fp_abort("Invalid port: %i\n", (int)port);
	}
...
uint32_t gpo_read(uint32_t port)
{
	...
	{
	case 0:
		return read_csr(CSR_UARCH4);
		break;
	case 1:
		return read_csr(CSR_UARCH5);
		break;
	case 2:
		return read_csr(CSR_UARCH6);
		break;
	case 3:
		return read_csr(CSR_UARCH7);
		break;
	default:
		_fp_abort("Invalid port: %i\n", (int)port);
	}
...
uint32_t gpi_read(uint32_t port)
{
	...
	{
	case 0:
		return read_csr(CSR_UARCH0);
		break;
	case 1:
		return read_csr(CSR_UARCH1);
		break;
	case 2:
		return read_csr(CSR_UARCH2);
		break;
	case 3:
		return read_csr(CSR_UARCH3);
		break;
	default:
		_fp_abort("Invalid port: %i\n", (int)port);
	}
